# -*- coding: utf-8 -*-
"""ConvolutionalAutoEncoder.ipynb
...
class AugConvAutoencoder(nn.Module):
    def __init__(self):
        super(AugConvAutoencoder, self).__init__()
...
model = AugConvAutoencoder().to(device)
...
# -----------------------
# Use training data (normal images) to compute the reconstruction error distribution.
model.eval()
train_errors = []
with torch.no_grad():
    for images in train_dataloader:
...
# You can also explicitly plot the threshold if it differs from mean + 4σ
...
labels_csv_path = "/mnt/anom_proj/data/New/augnormalssym/augmented_unsupe.csv"  # Update this if needed
labels_df = pd.read_csv(labels_csv_path)

# Create dict of filename -> label (0 = normal, 1 = anomaly)
...
# Randomly pick samples from validation set and plot/save them
...
def log_experiment_run(model, criterion, optimizer, train_dataloader, val_dataloader, test_dataloader,
    ...
    experiment_settings = {
        ...
        "model_parameters": model_parameters,           # Logging total parameter count
        "model_file_size_mb": model_file_size_mb          # Logging model file size in MB
    }
    ...
    return version_prefix  # so you can reuse versioned paths
