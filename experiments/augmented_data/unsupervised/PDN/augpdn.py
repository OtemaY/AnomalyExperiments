...
class AugPDN(nn.Module):
    def __init__(self, in_channels=1, out_channels=64):
        super(AugPDN, self).__init__()
...
# Model Instantiation
# -----------------------
# Instantiate the AugPDN (student) and the teacher feature extractor.
student = AugPDN(in_channels=1, out_channels=64).to(device)
teacher = TeacherFeatureExtractor(out_channels=64).to(device)

# Set teacher to evaluation mode (its parameters are frozen)
teacher.eval()

model = AugPDN().to(device)
...
epochs = 1  # Adjust the number of distillation epochs as needed
